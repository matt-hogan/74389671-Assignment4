...
function average (num1, num2, num3='missing') {
    return num3 == 'missing' ? Math.round((num1+num2)/2) : Math.round((num1+num2+num3)/3);
}

// 5) Create a function named leetSpeak that accepts a word
// as a parameter and returns the same word except all of the letter
// e occurrences (lower case only) are replaced by the number 3.
...
function leetSpeak (word) {
    return word.replace(/e/g, '3').replace(/a/g, '4');
}